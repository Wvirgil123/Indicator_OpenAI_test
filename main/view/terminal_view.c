...
static lv_obj_t *ui_wifi_list = NULL; // child of ui_screen_wifi, in wifi_list_init()
...
// void ui_btn_generate_event_init(void)
// {
//     lv_obj_add_event_cb(ui_screen_chatgpt, __ui_btn_generate_answer_config, LV_EVENT_CLICKED, NULL); // 按下的反应

//     // ui_generate_chatgpt_wait = lv_bar_create(ui_screen_chatgpt, 1000, 60); // must be type the btn, than shown
// }
...
static bool msgbox_is_showing = false;
static void close_msgbox_timer(lv_timer_t *timer)
{
    lv_obj_t *msgbox = (lv_obj_t *)timer->user_data;
    lv_obj_del(msgbox);
    lv_timer_del(timer);
    msgbox_is_showing = false;
}
void show_msgbox(const char *message)
{
    if (msgbox_is_showing == true)
    {
        return;
    }
    lv_obj_t *msgbox = lv_msgbox_create(lv_scr_act(), "Warning", message, NULL, true);
    msgbox_is_showing = true;
    lv_obj_set_size(msgbox, 360, 200);
    lv_obj_center(msgbox);
    // TODO 更改文字大小
    lv_timer_t *timer = lv_timer_create(close_msgbox_timer, 2000, msgbox); // 设置提示框在 2 秒后自动关闭
}
...
void ui_event_btn_go_chatgpt(lv_event_t *e)
{
    ...
    {
        if (openai_run_state.WIFI_CONNECTED && openai_run_state.OPENAI_SAVEDKEY) // The WiFi is connected.
        {
            ui_screen_last = ui_screen_OpenAI;
            _ui_screen_change(ui_screen_chatgpt, LV_SCR_LOAD_ANIM_MOVE_LEFT, 300, 0);
        }
        else if (openai_run_state.OPENAI_SAVEDKEY == false) // 提示用户写入 OpenAI, 提示框可自动2s后关闭,可用户关闭
        {
            // ESP_LOGI(TAG, " haven't set KEY!");
            show_msgbox("haven't set KEY!");
        }
        else
        {
            // ESP_LOGI(TAG, "Waiting for WIFI connected!");
            show_msgbox("Waiting for WIFI connected!");
        }
    }
}
extern lv_obj_t *ui_screen_DALLE1;
void ui_event_btn_go_dalle(lv_event_t *e)
{
    ...
    if (event_code == LV_EVENT_CLICKED)
    {
        ...
        {
            ui_screen_last = ui_screen_OpenAI;
            _ui_screen_change(ui_screen_DALLE1, LV_SCR_LOAD_ANIM_MOVE_LEFT, 300, 0);
        }
        else if (openai_run_state.OPENAI_SAVEDKEY == false) // 提示用户写入 OpenAI, 提示框可自动2s后关闭,可用户关闭
        {
            // ESP_LOGI(TAG, " haven't set KEY!");
...
void ui_event_btn_chatgpt_generate(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    lv_obj_t *target = lv_event_get_target(e);
    if (event_code == LV_EVENT_CLICKED)
    {
        ...
        // 获取文本框内容 lv_textarea_get_text，存储到 req.question，然后发送事件
        strncpy(req.question, question, sizeof(req.question));
        // TODO 显示一个 Wait 等待界面, 并隐藏按钮，隐藏输入框，并利用 Bar 进度条进行交互、
        
        esp_event_post_to(view_event_handle, VIEW_EVENT_BASE, VIEW_BAR_MIN, NULL, 0, portMAX_DELAY);
        // ESP_LOGI("YZP", "req.question: %s", req.question);
        esp_event_post_to(view_event_handle, VIEW_EVENT_BASE, VIEW_EVENT_CHATGPT_REQUEST, &req, sizeof(req), portMAX_DELAY); 

        usleep(1500); // 模拟
        esp_event_post_to(view_event_handle, VIEW_EVENT_BASE, VIEW_BAR_MID, NULL, 0, portMAX_DELAY);
        usleep(1500); // 模拟
        esp_event_post_to(view_event_handle, VIEW_EVENT_BASE, VIEW_BAR_MAX, NULL, 0, portMAX_DELAY);

        // lv_obj_add_flag(ui_chatgpt_wait, LV_OBJ_FLAG_HIDDEN);
    }
}
// TODO 發送 event 到 view_event_handle？用 chatgpt2 來處理？
// TODO 显示一个 Wait 等待界面, 并隐藏按钮，隐藏输入框，并利用 Bar 进度条进行交互、
// TODO Post 文本框内容、等待Request
// TODO 当收到回复后，显示回复信息，并销毁进度条


// esp post事件 VIEW_EVENT_CHATGPT_REQUEST
// DONE 按 OK 隐藏键盘
...
void chatgpt_txt_show(struct view_data_openai_show *p_chat_show)
{
    lv_label_set_text(ui_text_readonly_gpt_request, p_chat_show->question);
    lv_label_set_text(ui_text_readonly_gpt_response, p_chat_show->p_answer);
}
void ui_event_keyboard_load(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    lv_obj_t *target = lv_event_get_target(e);
    if(event_code == LV_EVENT_SCREEN_LOAD_START)
...
void ui_screen_btn_openai_init(void)
{
    lv_obj_add_event_cb(ui_btn_go_chatgpt, ui_event_btn_go_chatgpt, LV_EVENT_CLICKED, NULL);            // 点击 Go ChatGPT 按钮时，触发的事件
    lv_obj_add_event_cb(ui_btn_generate_answer, ui_event_btn_chatgpt_generate, LV_EVENT_CLICKED, NULL); // 点击 Generta Answer 按钮时，触发的事件
    lv_obj_add_event_cb(ui_GenerAnswer1, ui_event_btn_chatgpt_generate, LV_EVENT_CLICKED, NULL);        // 点击 Regenerta 按钮时，触发的事件
    lv_obj_add_event_cb(ui_screen_chatgpt2, ui_event_keyboard_load, LV_EVENT_SCREEN_LOAD_START, NULL);        // 点击 Regenerta 按钮时，触发的事件
...
/**
 * @brief
 * 当确认 WIFI 连接后，按下 Generta Answer 按钮时，触发的事件:
 * 1. 显示一个 Wait 等待界面, 并隐藏按钮，隐藏输入框，并利用 Bar 进度条进行交互、
 * 2. Post 文本框内容、等待Request
 * 3. 当收到回复后，显示回复信息，并销毁进度条
 */
// void __ui_btn_generate_answer_config(lv_event_t *e)
// {
//     lv_event_code_t event_code = lv_event_get_code(e);
//     lv_obj_t *target = lv_event_get_target(e);
//     lv_obj_t *ui_bar_generate_wait;
//     if (event_code == LV_EVENT_CLICKED)
//     {
//         // Check if WIFI is connected with ESP32

//         // 隐藏按钮，隐藏输入框
//         lv_obj_clear_flag(ui_btn_generate_answer, LV_OBJ_FLAG_HIDDEN);
//         lv_obj_clear_flag(ui_input_chatgpt, LV_OBJ_FLAG_HIDDEN);
//         // 显示等待界面
//         ui_bar_generate_wait = lv_bar_create(ui_screen_chatgpt, 1000, 60); // must be type the btn, than shown
//         lv_obj_clear_flag(ui_bar_generate_wait, LV_OBJ_FLAG_HIDDEN);
//         // Post 文本框内容、等待Request
//         esp_event_post_to(view_event_handle, VIEW_EVENT_BASE, VIEW_EVENT_CHATGPT_REQ, NULL, 0, portMAX_DELAY);
//     }
// }
/**************** END of OpenAI **********/
...
static void __view_event_handler(void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
{
   ...
   case VIEW_EVENT_TIME_CFG_UPDATE:
    ...
    case VIEW_EVENT_WIFI_LIST:
    {
        ...
        struct view_data_wifi_list *p_list = (struct view_data_wifi_list *)event_data; // 数据接收
    ...
    case VIEW_EVENT_SENSOR_DATA: // 會多次循環運行
    {
    ...
    case VIEW_EVENT_CHATGPT_VIEW: // first here
    {
        ESP_LOGI(TAG, "event: VIEW_EVENT_CHATGPT_VIEW");
        static struct view_data_openai_show *p_data;
        p_data = (struct view_data_openai_show *)event_data;
        ESP_LOGI(TAG, "Question: %s", p_data->question);
        ESP_LOGI(TAG, "Answer: %s", p_data->p_answer);
        chatgpt_txt_show(p_data);   
        // 跳转界面
        lv_disp_load_scr(ui_screen_chatgpt2);
        // 传递数据
        break;
    }
    /**
    ...
    case VIEW_BAR_MIN:
        /* code */
        ESP_LOGI(TAG, "Bar: Minimum");
        break;
    case VIEW_BAR_MID:
        ESP_LOGI(TAG, "Bar: Middle");
        break;
    case VIEW_BAR_MAX:
        ESP_LOGI(TAG, "Bar: Maximum");
        break;
    default:
        break;
    }
...
/**
 * 这段代码是在ESP32 ESP-IDF中注册一个事件处理程序，用于处理视图事件（View Event）。它通过使用ESP-IDF提供的事件驱动框架，让应用程序能够响应特定的事件。

该代码使用了一个for循环，将0到VIEW_EVENT_ALL的所有事件注册到视图事件处理程序（view_event_handle）中。具体来说，该代码为每个事件（i）注册了一个处理程序，该处理程序是__view_event_handler。此外，该代码使用ESP_ERROR_CHECK宏检查事件处理程序注册的返回值，以确保注册成功。

在ESP32 ESP-IDF中，事件驱动编程是一种常见的编程模型，它基于事件和事件处理程序之间的交互。通过使用事件驱动框架，应用程序可以更轻松地实现复杂的控制逻辑和状态转换。视图事件是ESP-IDF中的一个重要概念，用于管理显示屏的状态和交互。通过注册视图事件处理程序，应用程序可以响应不同的视图事件，例如按下按钮、滑动屏幕等。
*/