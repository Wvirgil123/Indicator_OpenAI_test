...
static int mbedtls_send_then_recv(char *p_server, char *p_port, 
                           char *p_tx, size_t tx_len, 
                           char *p_rx, size_t rx_len,
                           int  delay_ms)
{
    int ret, flags, len;
    char buf[512];
    ...
    if ((ret = mbedtls_net_connect(&server_fd, p_server,
    ...
    do {
        ret = mbedtls_ssl_write(&ssl,
                                (const unsigned char *)*p_tx + written_bytes,
    ...
    do {
        ret = mbedtls_ssl_read(&ssl, (unsigned char *)(p_rx + recv_len), rx_len);
...
static int __chat_json_prase(const char *p_str, char *p_answer, char * p_err)
{
    ...
    if( cjson_item != NULL ) {
        ...
        if( cjson_item1 != NULL  && cjson_item1->valuestring != NULL) {
            strncpy( p_err, cjson_item1->valuestring);
        }
        cJSON_Delete(root);
        return -1;
    }
    ...
    if( cjson_item != NULL  ) {
        ...
        if(cjson_item1 != NULL ) {
            ...
            if( cjson_item2 !=NULL && cjson_item2->valuestring != NULL ) {
                strncpy( p_answer, cjson_item2->valuestring);
                        ...
                        struct view_data_openai_response *p_resp)
{
    ...
    data_len = sprintf(data_buf, "{\"model\":\"gpt-3.5-turbo\",\"messages\":[{\"role\":\"user\",\"content\":\"%s\"}],\"temperature\":0.7}", p_req->question);
    ...
    memset(p_recv_buf, recv_buf_max_len);
    ret = mbedtls_send_then_recv(WEB_SERVER, WEB_PORT, request_buf, len, p_recv_buf, recv_buf_max_len, 100);
    if(ret < 0) {
...
static int __image_json_prase(const char *p_str, char *p_url, char * p_err)
{
    ...
    if( cjson_item != NULL  ) {
        ...
        if( cjson_item1 !=NULL && cjson_item1->valuestring != NULL ) {
            strncpy( p_url, cjson_item1->valuestring);
            cJSON_Delete(root);
            return 0;
        }
    }
    strncpy( p_err, "Not find url");
    return -1;
}
                           ...
                           struct view_data_openai_response *p_resp)
{
    ...
    data_len = sprintf(data_buf, "{\"prompt\":\"%s\",\"n\":1,\"size\":\"512x512\"}", p_req->question);
    ...
    memset(p_recv_buf, recv_buf_max_len);
    ret = mbedtls_send_then_recv(WEB_SERVER, WEB_PORT, request_buf, len, p_recv_buf, recv_buf_max_len, 2000);
    if(ret < 0) {
        ESP_LOGE(TAG,"mbedtls request fail");
        p_resp.ret = 0;
        strcpy(p_resp->err_msg, "Request fail");
...
static void __openai_api_key_read(void)
{
    ...
    ret = terminal_storage_read(OPENAI_API_KEY_STORAGE, (void *)openai_api_key, &len);
